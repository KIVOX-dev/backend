...
// files, which get folded in here as their private storage reference (see
// middlewares/upload.js#verifyAndPersist and utils/privateMedia.js). Ported from python-service's profile.py#_extract_values, which did
// the same content-type-based reconciliation manually.
const saveOwn = asyncHandler(async (req, res) => {
  const values = { ...req.body };
  for (const file of req.files || []) {
    values[file.fieldname] = file.storageRef;
  }
