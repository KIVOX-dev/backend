...
def create_batch(
    data: BatchCreateRequest, current_user=Depends(get_current_user), db=Depends(get_db)
):
    batch = {
    ...
    for student_data in data.students:
        email = (
            student_data.email
            or f"{student_data.roll or student_data.student_id}@upscaler-ai.local"
        ).lower()
...
def update_batch_status(
    ...
    db["batches"].update_one(
        {"id": batch_id}, {"$set": {"status": data.status, "updated_at": _now()}}
    )
