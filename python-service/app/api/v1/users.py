...
def create_user(
    payload: AdminUserCreateRequest, db=Depends(get_db), current_user=Depends(get_current_user)
):
...
def approve_user(user_id: int, db=Depends(get_db), current_user=Depends(superadmin_checker)):
...
def reject_user(user_id: int, db=Depends(get_db), current_user=Depends(superadmin_checker)):
    user = db["users"].find_one({"id": user_id})
    if not user:
        raise NotFoundError("User", str(user_id))
    db["users"].update_one(
