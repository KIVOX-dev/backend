...
def get_student_dashboard(
    student_id: int, db=Depends(get_db), current_user=Depends(get_current_user)
):
    assert_can_act_on_student(current_user, student_id, db)
    profile = db["student_profiles"].find_one({"user_id": student_id})
    if not profile:
    ...
    recent_attempts = list(
        db["assessment_attempts"].find({"student_id": student_id}).sort("created_at", -1).limit(5)
    )
    recent_activity = []
...
def log_student_test(
    student_id: int, data: dict, db=Depends(get_db), current_user=Depends(get_current_user)
):
    assert_can_act_on_student(current_user, student_id, db)
...
def log_student_interview(
    ...
    db["student_profiles"].update_one(
        {"user_id": student_id}, {"$inc": {"interviews_completed": 1}}
    )
...
def create_batch_students(
    ...
    for item in students:
        email = (
            item.get("email") or f"{item.get('roll') or item.get('studentId')}@upscaler-ai.local"
        )
    ...
    return MessageResponse(
        message=f"Successfully onboarded {created} students. Status: {default_status}"
    )
